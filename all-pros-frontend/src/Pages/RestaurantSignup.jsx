  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Navigate to dashboard and pass restaurant data via router state
        setTimeout(() => {
          navigate('/dashboard', { state: { restaurant: response.data.restaurant } });
        }, 2000);
      } catch (error) {
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 4 }}>
              <Box
                sx={{
                  ...
                  justifyContent: 'center',
                  alignItems: 'center',
                  boxShadow: '0 10px 25px rgba(127, 29, 29, 0.5)',
                  mb: 2,
