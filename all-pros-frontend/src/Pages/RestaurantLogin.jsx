...
const RestaurantLogin = () => {
  ...
  // Function to navigate after login (e.g., to verification page or directly to dashboard)
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Save the full restaurant data in localStorage under "restaurantData"
        if (response.data.restaurantData && response.data.restaurantData._id) {
          localStorage.setItem(
            'restaurantData',
            JSON.stringify(response.data.restaurantData)
          );
        } else {
          console.error('Restaurant data missing in response:', response.data);
          setSnackbarMessage('Login successful, but restaurant data is missing.');
        }
                ...
                <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                  <Button
                    ...
                    sx={{
                      ...
                      fontSize: '1.1rem',
                      '&:hover': {
                        background: 'linear-gradient(135deg , #A0522D, rgba(163, 12, 12, 0.85))',
