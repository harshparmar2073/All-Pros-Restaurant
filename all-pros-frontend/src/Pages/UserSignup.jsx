...
const UserSignup = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        // Create a FormData object to support file upload
        ...
        // If your API expects a field to confirm password, you may send it
        formData.append('confirmPassword', values.confirmPassword);
        formData.append('termsAccepted', values.termsAccepted ? 'true' : 'false');

        // Append additional data (stringify arrays/objects if necessary)
        formData.append('experiences', JSON.stringify(experiences));
        formData.append('education', JSON.stringify(education));
        formData.append('schoolEducation', JSON.stringify(schoolEducation));

        // Append resume file if available (note: the field name must match your Multer configuration)
        if (resumeFile) {
        ...
        // Optionally redirect after a delay (use your routing solution)
        setTimeout(() => {
          navigate('/user-dashboard'); // Uncomment and use your navigation method
        }, 2000);
      } catch (error) {
